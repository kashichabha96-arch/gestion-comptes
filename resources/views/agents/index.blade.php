...
<body class="bg-gray-100 p-6 font-sans">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mt-6 mb-8 flex justify-between items-center animate__animated animate__fadeIn">
            ...
            <!-- Ajouter Utilisateur -->
            <div class="flex items-center gap-2 bg-purple-600 hover:bg-purple-700
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white">
                    <tr>
                        ...
                        <th class="py-3 px-4 text-left">Agence</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b hover:bg-gray-50 transition animate__animated animate__fadeInUp">
                        <td class="py-3 px-4 font-medium">{{ $agent->nom }}</td>
                        <td class="py-3 px-4">{{ $agent->prenom ?? '-' }}</td>
                        <td class="py-3 px-4">{{ $agent->date_naissance ?? '-' }}</td>
                        <td class="py-3 px-4">{{ $agent->adresse ?? '-' }}</td>
                        <td class="py-3 px-4 text-blue-600">{{ $agent->email }}</td>
                        <td class="py-3 px-4">{{ $agent->telephone ?? '-' }}</td>
                        <td class="py-3 px-4">{{ $agent->agence ?? '-' }}</td>
                        <!-- Actions -->
                        <td class="py-3 px-4 space-x-2 flex justify-center">
                            ...
                            <form action="{{ route('agents.destroy', $agent->id) }}" method="POST"
                                onsubmit="return confirm('Êtes-vous sûr de supprimer cet utilisateur ?');"
                                class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
